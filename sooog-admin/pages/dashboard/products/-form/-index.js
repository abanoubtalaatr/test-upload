...
export default {
  ...
  // },
  created() {
    console.log('countrie', this.countries)
  ...
  methods: {
    ...
    async handleCategoryRelations(id) {
      ...
      this.properties = response[1].map((obj) => {
        ...
        if (this.param_id) {
          ...
          if (property_value)
            obj.value = typeof property_value.option_value == 'object' ? property_value.option_value?.id : property_value.option_value
        }
        return obj
      })
      if (this.param_id)
    ...
    back() {
      this.$router.push(this.localePath({name: "dashboard-products"}))
    }
  },
}
