...
export default {
    async asyncData (context){
        let response = await context.$axios.$get("/admin/stores?is_paginated=1&type=centers&status=new").catch((e) => { console.log('err: ', e) })
        return {
    ...
    data () {
        return {
            titlePage: this.$t('admin.centers'),
            orderBy: 'id',
            orderType: 'DESC',
            fieldsData: [
                {
    ...
    methods: {
        ...
        */
        ...
        this.queryParam = `?page=${this.meta.current_page}&is_paginated=1&status=new&type=centers&public_search=${this.publicSearch}&orderBy=${this.orderBy}&orderType=${this.orderType}`
        ...
        */
        ...
        exportToExcel(){
            StoreService.excelExport(`?type=centers&status=new&public_search=${this.publicSearch}`)
            .then(response => {
          ...
          forceFileDownload(response){
            ...
            link.href = url
            link.setAttribute('download', 'centers_requests.xlsx') //or any other extension
    ...
    computed: {
        titleStack () {
            return [this.$t('admin.centers_requests')]
        }
    },
    head () {
        return {
