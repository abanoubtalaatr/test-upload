      ...
      <div class="table_wrap">
        ...
        >
          ...
          <template v-slot:cell(action)="el">
            ...
            <span class="table_icon mr-2" @click.prevent="handleStatus(el.item, 'accepted')" v-show="permissions.includes('centers.update')"
              :title="$t('admin.acceptTitle')">
              <i class="green fa fa-check-square" />
            </span>
            <span class="table_icon mr-2" @click.prevent="handleStatus(el.item, 'rejected')" v-show="permissions.includes('centers.update')"
              :title="$t('admin.rejectTitle')">
              <i class="red fa fa-times-circle" />
            </span>

            <nuxt-link class="table_icon" :to="localePath(`/dashboard/centers-requests/${el.item.id}`)"
