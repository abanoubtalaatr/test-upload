...
export default {
    ...
    async asyncData (context){
        ...
        if(context.query.customer_id){
            response = await context.$axios.$get(`/admin/orders?type=centers&is_paginated=1&customer=${context.query.customer_id}`).catch((e) => { console.log('err: ', e) })
        }else{
            response = await context.$axios.$get("/admin/orders?type=centers&is_paginated=1").catch((e) => { console.log('err: ', e) })
        }
        return {
