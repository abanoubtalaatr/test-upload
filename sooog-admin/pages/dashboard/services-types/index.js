import CategoryService from "~/pages/dashboard/categories/-service/-CategoryService.js";
export default {
    async asyncData (context){
        //let response = await CategoryService.getAll()
        let response = await context.$axios.$get("/admin/categories?is_paginated=1&is_detailed=1&type=centers&orderBy=id&orderType=desc").catch((e) => { console.log('err: ', e) })
    ...
    data () {
        return {
            titlePage: this.$t('admin.services_types'),
            orderBy: 'id',
            orderType: 'DESC',
            fieldsData: [
                {
            ...
            customEvents: [
                ...
                { eventName: 'event-delete-service_type', callback: this.handleDelete }
            ],
    ...
    methods: {
        ...
        */
        ...
        this.queryParam = `?page=${this.meta.current_page}&is_paginated=1&is_detailed=1&type=centers&orderBy=id&orderType=desc&public_search=${this.publicSearch}&orderBy=${this.orderBy}&orderType=${this.orderType}`

        await CategoryService.getAll(this.queryParam)
            .then((response) => {
    ...
    computed: {
        titleStack () {
            return [this.$t('admin.services_types')]
        }
    },
    head () {
        return {
