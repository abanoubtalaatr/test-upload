<template>
    <card-component :title="item.name" icon="fas fa-clipboard-list 5x">
            ...
            <b-row>
                <b-col lg="6">
                    ...
                    <p>
                        ...
                        <nuxt-link :to="localePath(`/dashboard/users/${item.user.id}`)">{{ item.user.name }}</nuxt-link>
                    </p>
                ...
                <b-col lg="6" v-if="type=='centers'">
                    <p><span style="color:green;">{{$t('admin.service') + ' : '}}</span>  {{ item.item.service.name }}</p>
                    <p><span style="color:green;">{{$t('admin.preview_fees') + ' : '}}</span>  {{ item.item.preview_fees }}</p>
                    <p><span style="color:green;">{{$t('admin.service_price') + ' : '}}</span>  {{ item.item.service_price }}</p>
                    <p><span style="color:green;">{{$t('admin.category') + ' : '}}</span>  {{ item.item.subcategory.name }}</p>
                    <p><span style="color:green;">{{$t('admin.problem_description') + ' : '}}</span>  {{ item.item.problem_description }}</p>
                    <p><span style="color:green;">{{$t('admin.promo_code_discount') + ' : ' }}</span>  {{item.promo_code_discount }}</p>
                    <p><span style="color:green;">{{$t('admin.subtotal') + ' : ' }}</span>  {{item.subtotal }}</p>
                    <p><span style="color:green;">{{$t('admin.order_added_tax') + ' : ' }}</span>  {{item.order_added_tax }}</p>
                    <p><span style="color:green;">{{$t('admin.total') + ' : ' }}</span>  {{item.total }}</p>
          ...
          <div class="level" v-if="item.payment_method.type == 'bank_transfer'">
            <b-row>
                <b-col lg="12">
                    ...
                    <p><span style="color:green;">{{$t('admin.deposit_receipt') + ' : '}}</span>
                    <a target="_blank" :href="item.bank_transfer.deposit_receipt">
                        ...
                        :data="item.bank_transfer.deposit_receipt" class="img-thumbnail" /> -->
                        <i class="fas fa-file red" v-if="item.bank_transfer.file_type == 'pdf'"></i>
                        <b-img
                            v-else
        ...
        <div class="text-center mt-3 mb-2">
            ...
            <a target="_blank" :href="item.invoice" class="btn btn-success" v-if="item.invoice != ''">
