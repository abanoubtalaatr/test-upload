...
import FilterModal from '~/pages/stores/products/-filter/-index.vue'
...
export default {
  ...
  middleware({ redirect, app }) {
    ...
    if (!app.$cookies.get("storePermissions").includes('products.index')) {
      return redirect(app.localePath('stores-403'))
    }
  },
    async asyncData (context){
        try {
            let response = await context.$axios.$get("/store/products?type=stores&is_paginated=1&is_detailed=1")
            return {
