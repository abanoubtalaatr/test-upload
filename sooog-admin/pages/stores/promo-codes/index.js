...
export default {
  ...
  middleware({ redirect, app }) {
    ...
    if (!app.$cookies.get("storePermissions").includes('promo_codes.index')) {
      return redirect(app.localePath('stores-403'))
    }
  },
    async asyncData (context){
        let response = await context.$axios.$get("/store/promo-codes?is_paginated=1").catch((e) => { console.log('err: ', e) })
        return {
        ...
        return {
          store:null,
            ...
            customEvents: [
                ...
                { eventName: 'event-delete-promoCode', callback: this.handleDelete }
            ],
    ...
    methods: {
        ...
        */
        ...
        await PromoCodeService.getAll(this.queryParam)
            .then((response) => {
        ...
        */
        ...
        await PromoCodeService.destroy(id)
            .then(() => {
            //* remove this row *//
        ...
        PromoCodeService.toggleStatus(id)
            .then((response) => {
            //* update list *//
        ...
        exportToExcel(){
            PromoCodeService.excelExport(`?public_search=${this.publicSearch}`)
            .then(response => {
          ...
          forceFileDownload(response){
            ...
            link.href = url
            link.setAttribute('download', 'promo-codes.xlsx') //or any other extension
    ...
    computed: {
      ...mapState({
        ...
        titleStack () {
            return [this.$t('admin.promoCodes')]
        }
    },
    head () {
        return {
