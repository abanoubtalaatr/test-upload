...
import filterModal from '~/pages/stores/product-orders/-filter/-index.vue'
...
export default {
  ...
  async asyncData(context) {
    let response = await context.$axios.$get("/store/refunds?type=stores&is_paginated=1").catch((e) => {
      console.log('err: ', e)
    })
    return {
  ...
  data() {
    return {
      ...
      fieldsData: [
        ...
        {
          key: "order.user_address.city.name",
  ...
  methods: {
    async doFilterOrders(search) {
      ...
      await RefundService.getAll(`?${search}`)
        .then((response) => {
    ...
    */
    async loadAsyncData() {
      ...
      await RefundService.getAll(this.queryParam)
        .then((response) => {
          this.collection = response.data
    ...
    */
    ...
    async handleDelete(id) {
      await RefundService.destroy(id)
        .then(() => {
          //* remove this row *//
    ...
    changeStatus(item) {
      RefundService.changeStatus(item.id, {status: this.status, reason: item.reason})
        .then((response) => {
          //* update list *//
    ...
    exportToExcel() {
      RefundService.excelExport(`?public_search=${this.publicSearch}`)
        .then(response => {
    ...
    forceFileDownload(response) {
      ...
      link.href = url
      link.setAttribute('download', 'refunds.xlsx') //or any other extension
