...
export default {
  ...
  middleware({ redirect, app }) {
    ...
    if (!app.$cookies.get("storePermissions").includes('warranties.index')) {
      return redirect(app.localePath('stores-403'))
    }
  },
    async asyncData (context){
        //let response = await WarrantyService.getAll()
        let response = await context.$axios.$get("/store/warranties?is_paginated=1").catch((e) => { console.log('err: ', e) })
        return {
    ...
    data () {
        return {
            ...
            customEvents: [
                ...
                { eventName: 'event-delete-warranty', callback: this.handleDelete }
            ],
    ...
    methods: {
        ...
        */
        ...
        await WarrantyService.getAll(this.queryParam)
            .then((response) => {
        ...
        */
        ...
        await WarrantyService.destroy(id)
            .then(() => {
            //* remove this row *//
        ...
        WarrantyService.toggleStatus(id)
            .then((response) => {
            //* update list *//
    ...
    computed: {
        titleStack () {
            return [this.$t('admin.warranties')]
        }
    },
    head () {
        return {
