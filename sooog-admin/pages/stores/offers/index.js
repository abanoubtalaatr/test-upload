...
import FilterModal from '~/pages/stores/offers/-filter/-index.vue'
...
export default {
  ...
  middleware({ redirect, app }) {
    ...
    if (!app.$cookies.get("storePermissions").includes('offers.index')) {
      return redirect(app.localePath('stores-403'))
    }
  },
    async asyncData (context){
        let response = await context.$axios.$get("/store/offers?is_paginated=1").catch((e) => { console.log('err: ', e) })
        return {
