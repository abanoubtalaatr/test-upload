...
export default {
  ...
  middleware({ redirect, app }) {
    ...
    if (!app.$cookies.get("storePermissions").includes('orders.index')) {
    ...
    async asyncData (context){
        ...
        if(context.query.customer_id){
            response = await context.$axios.$get(`/store/orders?type=stores&is_paginated=1&customer=${context.query.customer_id}`).catch((e) => { console.log('err: ', e) })
        }else{
            response = await context.$axios.$get("/store/orders?type=stores&is_paginated=1").catch((e) => { console.log('err: ', e) })
        }
            return {
