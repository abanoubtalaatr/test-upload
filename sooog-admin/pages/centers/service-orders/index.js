...
import filterModal from '~/pages/centers/service-orders/-filter/-index.vue'
export default {
  layout: 'center',
  middleware({redirect, app}) {
    ...
    async asyncData (context){
        let response = null
        if(context.query.customer_id || context.query.customer_id!=undefined){
            response = await context.$axios.$get(`/center/orders?type=centers&is_paginated=1&customer=${context.query.customer_id}`).catch((e) => { console.log('err: ', e) })
        }else{
            response = await context.$axios.$get("/center/orders?type=centers&is_paginated=1").catch((e) => { console.log('err: ', e) })
        }
        return {
    ...
    methods: {
        ...
        */
        ...
        this.queryParam = `?type=centers&page=${this.meta.current_page}&is_paginated=1&public_search=${this.publicSearch}&orderBy=${this.orderBy}&orderType=${this.orderType}`
        ...
        */
        ...
        exportToExcel(){
          ...
          forceFileDownload(response){
            ...
            link.href = url
            link.setAttribute('download', 'service_orders.xlsx') //or any other extension
