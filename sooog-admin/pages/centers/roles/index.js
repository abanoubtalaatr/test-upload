import RoleService from "~/pages/centers/roles/service/RoleService.js";

export default {
  layout: 'center',
  middleware({redirect, app}) {
    // If the user is not authorized
    if (!app.$cookies.get("centerPermissions").includes('roles.index')) {
      return redirect(app.localePath('stores-403'))
    }
  },
  async asyncData (context) {

    const response = await context.$axios.$get('/center/roles').catch(() => {})
