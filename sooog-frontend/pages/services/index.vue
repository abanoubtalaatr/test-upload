    ...
    <main class="main-content">
      ...
      <section class="filter custom-padd">
        <div class="container">
          <div class="row">
             <div class="col-md-3">
              <div class="h3">{{products.length && this.$route.query.store ? products[0].store.name : $t('front.services') }}
                <span class="blue">({{meta.total}}) {{$t('front.service')}}</span></div>
            </div>
              ...
              <div class="row">
                ...
                <div class="col-6 col-md-3 padd-r-0">
                  <select class="form-control select-g" name="category" v-model="filter.category" @change="changeSelect">
                    <option value="" selected disabled>{{$t('front.display_all_service_types')}}</option>
        ...
        <div class="container">
          <div class="row" v-if="products.length">
            <div v-for="(product, key) in products" :key="key" class="col-6 col-md-3">
              <!-- <nuxt-link :to="localePath(`/services/${product.id}`)"> -->
