...
export default {
  ...
  async asyncData (context) {
    let queryParam = `?type=centers&is_paginated=1`
    ...
    const response = await Promise.all([
      ...
      context.$axios.$get(`/categories?type=centers`).catch(() => {}),
    ...
    return {
      ...
      meta: response[1].meta,
      links: response[1].links
    }
  },
  data() {
    return {
      titlePage: this.$t('front.services'),
      queryParam: '',
      filter: {
        price_from: 0,
        price_to: 20000,
