...
class RequestOfferQuantityResponder extends Responder
{
    ...
    public function respond()
    {
        ...
        if ($this->response->getStatus() == Response::HTTP_CREATED) {
            return $this->sendJson(
                new RequestOfferQuantityResource($this->response->getData()),
                Response::HTTP_OK
            );
        }
        ...
        if ($this->response->getStatus() == Response::HTTP_OK)
                return $this->sendJson(
                    RequestOfferQuantityResource::collection($this->response->getData()),
                    $this->response->getStatus()
                );

        if ($this->response->getStatus() == Response::HTTP_PARTIAL_CONTENT)
            return $this->sendJson(
                RequestOfferQuantityResource::make($this->response->getData()),
                $this->response->getStatus()
            );
